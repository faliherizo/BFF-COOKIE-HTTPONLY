...
/**
 * Lists active session keys (using the specified key prefix "network").
 */
export async function listActiveSessions(): Promise<string[]> {
  try {
    const keys = await redisClient.keys('network:*');
    return keys;
  } catch (error) {
...
export async function invalidateSession(sessionId: string): Promise<void> {
  try {
    await redisClient.del(`network:${sessionId}`);
  } catch (error) {
...
export async function refreshSession(sessionId: string, newTTL: number): Promise<void> {
  try {
    await redisClient.expire(`network:${sessionId}`, newTTL);
  } catch (error) {
